...
enum DataType { POSITION, COLOR, UV, TEXID };
...
class Mesh {
  ...
  // instanceVBOs[1]:r,g,b,a颜色,
  // instanceVBOs[2]:u,v贴图坐标,
  // instanceVBOs[3]:texid贴图索引)
  GLuint VBO, VAO, instanceVBOs[4], FBO, TBO;
  // 包含的图形
...
public:
  ...
  // 绘制矩形(材质默认混合颜色并填充到矩形)
  void drawquad(glm::vec2 &cp, float w, float h, glm::vec4 &color,
  ...
  void drawquad(glm::vec2 &&cp, float w, float h, glm::vec4 &&color,
                glm::vec2 &screen_size);
  // 绘制矩形(材质默认填充到矩形)
  void drawquad(glm::vec2 &cp, float w, float h,
                std::shared_ptr<Texture> &texture, TexType texture_type,
                glm::vec2 &screen_size);
  void drawquad(glm::vec2 &&cp, float w, float h,
                std::shared_ptr<Texture> &texture, TexType texture_type,
