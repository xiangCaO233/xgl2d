...
void MaxRectsBinPack::Insert(std::vector<RectSize> &rects,
    ...
    while (rects.size() > 0) {
        ...
        for (size_t i = 0; i < rects.size(); ++i) {
            int score1;
            int score2;
            Rect newNode = ScoreRect(rects[i].width, rects[i].height, method,
                                     score1, score2);
    ...
    for (size_t i = 0; i < freeRectangles.size(); ++i) {
        ...
        if (freeRectangles[i].width >= height &&
            ...
            int flippedLongSideFit =
                max(flippedLeftoverHoriz, flippedLeftoverVert);
    ...
    for (size_t i = 0; i < freeRectangles.size(); ++i) {
        ...
        if (freeRectangles[i].width >= width &&
            ...
            int score = ContactPointScoreNode(
                freeRectangles[i].x, freeRectangles[i].y, width, height);
        ...
        if (freeRectangles[i].width >= height &&
            freeRectangles[i].height >= width) {
            int score = ContactPointScoreNode(
                freeRectangles[i].x, freeRectangles[i].y, height, width);
            if (score > bestContactScore) {
                bestNode.x = freeRectangles[i].x;
                bestNode.y = freeRectangles[i].y;
                bestNode.width = height;
