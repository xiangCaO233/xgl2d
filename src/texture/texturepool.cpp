...
Texturepool::Texturepool(std::string &texturedir, Shader *shader) : _shader(shader) {
    ...
    static unsigned char whitePixel[4 * 9] = {255, 255, 255, 255,};
...
void Texturepool::loadtexture(std::string &texturedir) {
    if (!is_done) {
        if (!std::filesystem::exists(texturedir)) {
            ...
            unsigned char *data = stbi_load(std::filesystem::absolute(texturedir).c_str(), &twidth, &theight,
                                            &nrChannels, 0);
            // 读取到表中
            ...
            LOG_WARN("文件(路径)[" + std::filesystem::absolute(texturedir).string() + "]不存在");
        }
    } else {
...
void Texturepool::creatatlas() {
    ...
    // 生成atlas图像
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, pack.binWidth, pack.binHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
    _vwidth = pack.binWidth;
    _vheight = pack.binHeight;
    LOG_INFO("当前纹理位置信息:");
    // 填充并释放纹理数据
    for (auto &pair: _texmetas) {
        std::string dbgstr = pair.first + "---res tex pos:[x:" + std::to_string(pair.second->woffset) + ",y:" +
                             std::to_string(pair.second->hoffset) + "]";
        LOG_INFO(dbgstr);
        // 放置纹理到atlas
        glTexSubImage2D(GL_TEXTURE_2D, 0, pair.second->woffset, pair.second->hoffset, pair.second->width,
                        pair.second->height, GL_RGBA, GL_UNSIGNED_BYTE, _texdatas[pair.second]);
    }
    // 绘制默认meta纹理
    glTexSubImage2D(GL_TEXTURE_2D, 0, _defmeta->woffset, _defmeta->hoffset, _defmeta->width, _defmeta->height, GL_RGBA,
                    GL_UNSIGNED_BYTE, _texdatas[_defmeta]);
    LOG_INFO("最终纹理集大小:" + std::to_string(pack.binWidth) + "x" + std::to_string(pack.binHeight) + "]");
    ...
    for (auto &meta: _metalist) {
        LOG_INFO("导入meta:[" + meta->name + "],id[" + std::to_string(meta->metaid) + "]");
    ...
    // 为 UTBO 分配数据空间
    glBufferData(GL_UNIFORM_BUFFER, (int) (utbodata.size() * sizeof(float)), utbodata.data(), GL_STATIC_DRAW);
