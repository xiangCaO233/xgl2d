...
Mesh::Mesh(Shader *shader, std::string &texture_dir, int oval_segment,
    ...
    : _shader(shader),
      _oval_segment(oval_segment),
      max_quad_count(initial_shape_count) {
    // 初始化缓冲对象
    ...
    // 上传原始矩形数据到缓冲区(VBO中)
    glBufferData(GL_ARRAY_BUFFER, (int)(vertices.size() * sizeof(float)),
    ...
    glBufferData(GL_ARRAY_BUFFER, (int)(max_quad_count * 11 * sizeof(float)),
    ...
    // 绑定尺寸缓冲(x和y的缩放倍率)
    // 描述location3 为2*float(max_quad_count * 2 ~ max_quad_count *
    // 4float内存块)
    ...
    // 绑定颜色缓冲
    // 描述location5 为4*float(max_quad_count * 5 ~ max_quad_count *
    // 9float内存块)
    ...
    // 绑定纹理元数据索引缓冲
    // 描述location6 为float(max_quad_count * 9 ~ max_quad_count *
    // 10float内存块)
...
void Mesh::drawquad(glm::vec2 &cp, float w, float h, float rotation,
	...
	bool needNewSequence =
	    ...
	    !_consequent_shapes.back().back()->is_shape_equal(shapetype);
	if (needNewSequence) {
	    // 创建新序列
	...
	std::shared_ptr<Quad> quad = std::make_shared<Quad>(
	    cp, w, h, rotation, color, texmeta, texture_type, shapetype);
	...
	update_consecutive(_update_consequent_shapes_poss, consequent_quads_pos,
			   _update_consequent_shapes_pos_datas,
			   consequent_quads_pos_data, quad, Datatype::POSITION);
	update_consecutive(_update_consequent_shapes_sizes,
			   consequent_quads_size,
			   _update_consequent_shapes_size_datas,
			   consequent_quads_size_data, quad, Datatype::SIZE);
	update_consecutive(
	    _update_consequent_shapes_rotations, consequent_quads_rotation,
	    _update_consequent_shapes_rotation_datas,
	    consequent_quads_rotation_data, quad, Datatype::ROTATION);
	update_consecutive(_update_consequent_shapes_colors,
			   consequent_quads_color,
			   _update_consequent_shapes_color_datas,
			   consequent_quads_color_data, quad, Datatype::COLOR);
	update_consecutive(
	    _update_consequent_shapes_texmetas, consequent_quads_texmeta,
	    _update_consequent_shapes_texmeta_datas,
	    consequent_quads_texmeta_data, quad, Datatype::TEXMETA);
	update_consecutive(_update_consequent_shapes_uvargs,
			   consequent_quads_uvarg,
			   _update_consequent_shapes_uvarg_datas,
	...
	if (!_handle_quad->is_cp_equal(cp)) {
	    _handle_quad->_cp = cp;
	    update_consecutive(
		_update_consequent_shapes_poss, consequent_quads_pos,
		_update_consequent_shapes_pos_datas, consequent_quads_pos_data,
		_handle_quad, Datatype::POSITION);
	}
	if (!_handle_quad->is_size_equal(w, h)) {
	    _handle_quad->_w = w;
	    _handle_quad->_h = h;
	    update_consecutive(
		_update_consequent_shapes_sizes, consequent_quads_size,
		_update_consequent_shapes_size_datas,
		consequent_quads_size_data, _handle_quad, Datatype::SIZE);
	}
	if (!_handle_quad->is_rotation_equal(rotation)) {
	    _handle_quad->_rotation = rotation;
	    update_consecutive(_update_consequent_shapes_rotations,
			       consequent_quads_rotation,
			       _update_consequent_shapes_rotation_datas,
			       consequent_quads_rotation_data, _handle_quad,
			       Datatype::ROTATION);
	}
	if (!_handle_quad->is_color_equal(color)) {
	    _handle_quad->_color = color;
	    update_consecutive(
		_update_consequent_shapes_colors, consequent_quads_color,
		_update_consequent_shapes_color_datas,
		consequent_quads_color_data, _handle_quad, Datatype::COLOR);
	}
	if (!_handle_quad->is_tex_equal(texmeta)) {
		...
		_update_consequent_shapes_texmetas, consequent_quads_texmeta,
		_update_consequent_shapes_texmeta_datas,
		consequent_quads_texmeta_data, _handle_quad, Datatype::TEXMETA);
	}
	if (!_handle_quad->is_textype_equal(texture_type)) {
	    _handle_quad->_textype = texture_type;
	    update_consecutive(
		_update_consequent_shapes_uvargs, consequent_quads_uvarg,
		_update_consequent_shapes_uvarg_datas,
		consequent_quads_uvarg_data, _handle_quad, Datatype::UVARG);
	}
	if (!_handle_quad->is_shape_equal(shapetype)) {
		...
		// 加入前一个列表
		_consequent_shapes[_handle_quad->updateindexlist - 1]
		    .emplace_back(_handle_quad);
		// 更改对应的元素标识
		_handle_quad->updateindexlist--;
		_handle_quad->updateindexelement =
		    (int)(_consequent_shapes[_handle_quad->updateindexlist]
			      .size());
	    } else {
		// 新增更新列表
		_consequent_shapes.emplace_back();
...
void Mesh::drawquad(glm::vec2 &cp, float w, float h, float rotation,
    ...
    drawquad(cp, w, h, rotation, {1.0f, 1.0f, 1.0f, 1.0f}, texture,
...
void Mesh::drawoval(glm::vec2 &cp, float rx, float ry, float rotation,
		    glm::vec4 &color, glm::vec2 &screen_size) {
    drawoval(cp, rx, ry, rotation, color, _texpool->_defmeta, FILL,
	     screen_size);
}
...
void Mesh::finish() {
	...
	// 删除此shape之后全部的更新列表
	_consequent_shapes.erase(
	    _consequent_shapes.begin() + shape->updateindexlist + 1,
	...
	for (int i = 0; i < _update_consequent_shapes_rotations.size(); i++) {
		...
		quads.back()->model_data_offset[2] -
		consequent_data_start_offset + sizeof(float);
	    glBufferSubData(GL_ARRAY_BUFFER, consequent_data_start_offset,
	...
	for (int i = 0; i < _update_consequent_shapes_colors.size(); i++) {
		...
		quads.back()->model_data_offset[3] -
		consequent_data_start_offset + 4 * sizeof(float);
	    glBufferSubData(GL_ARRAY_BUFFER, consequent_data_start_offset,
	...
	for (int i = 0; i < _update_consequent_shapes_texmetas.size(); i++) {
		...
		quads.back()->model_data_offset[4] -
		consequent_data_start_offset + sizeof(float);
	    glBufferSubData(GL_ARRAY_BUFFER, consequent_data_start_offset,
	...
	for (int i = 0; i < _update_consequent_shapes_uvargs.size(); i++) {
		...
		quads.back()->model_data_offset[5] -
		consequent_data_start_offset + sizeof(float);
	    glBufferSubData(GL_ARRAY_BUFFER, consequent_data_start_offset,
