...
public:
    // 构造Quad
    Quad(glm::vec2 &cp, float w, float h, float rotation, glm::vec4 &color, std::shared_ptr<TextureMeta> &tex,
         TexType texture_type, ShapeType shapetype) : _cp(cp), _w(w), _h(h), _rotation(rotation), _color(color),
    ...
    inline bool operator==(const Quad &q) const {
        return _cp == q._cp && _w == q._w && _h == q._h && _rotation == q._rotation && _color == q._color &&
               _texmeta == q._texmeta && _textype == q._textype && _shapetype == q._shapetype;
    };

    inline bool equals(const glm::vec2 &cp, const float w, const float h, const float rotation, const glm::vec4 &color,
                       const std::shared_ptr<TextureMeta> &tex, const TexType texture_type, ShapeType shapetype) {
        return _cp == cp && _w == w && _h == h && _rotation == rotation && _color == color && _texmeta == tex &&
               _textype == texture_type && _shapetype == shapetype;
    }
