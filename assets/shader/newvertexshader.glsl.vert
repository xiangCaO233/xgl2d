...
// 矩形数据// 图形中心点坐标
layout (location = 2) in vec2 shape_cp;// 图形的尺寸
layout (location = 3) in vec2 shape_size;// 图形旋转角度(角度)
layout (location = 4) in float shape_rotation;// 图形填充颜色
layout (location = 5) in vec4 shape_color;// 图形的纹理id(用于多纹理选择)
layout (location = 6) in float shape_texmetaid;// 图形的纹理变换
layout (location = 7) in float shape_uvarg;

// 视图矩阵
uniform mat4 viewmat;// 投影矩阵
uniform mat4 projmat;

const float PI = atan(1.0) * 4;

struct TextureMeta{ // 纹理元数据
    float woffset;
    float hoffset;
    float width;
    float height; };
...
layout(std140) uniform TextureMetaBuffer { float atlas_width;
    float atlas_height;
...
out vec4 vertex_color;// 所使用的纹理在纹理集中的位置uv
out vec2 texcoord;
out vec2 shapesize;
out float texuvarg;
...
void main(){ // 缩放矩形到指定大小
    vec2 scaled_pos = vpos.xy * shape_size * 0.5;// 旋转矩形
    ...
    shapesize = shape_size;// 应用视图和投影矩阵
    gl_Position = projmat * vec4(final_pos, 0.0, 1.0);
    vertex_color = shape_color;
    debug_var = shape_uvarg;// UV// 元数据id,定义了当前图元绑定纹理在纹理集中的宽高偏移和大小
    ...
    if (metaid == 0){ return; } else { texuvarg = shape_uvarg;
        ...
        float v = vuv.y * normalizedh + normalizedy;// 当前顶点对应纹理无视宽高在纹理集中一一对应位置(直接使用就是填充到整个矩形)
        vec2 uv = vec2(u, v);// 当前顶点对应纹理在纹理集中的相对尺寸
        vec2 tuv = vec2(normalizedw, normalizedh);

        // 填充方式参数
        int uvarg = int(shape_uvarg + 0.5);// 选择填充方式
        switch (uvarg){ case FILL:{ // 直接使用归一化uv
            texcoord = uv;
            break; }
            case REAPEAT:{ // 传递超出边界的uv
                ...
                texcoord  = vec2(ru, rv);
                break; }
            case REAPEAT_BY_CENTER:{ // 计算当前顶点矩形的中心点偏移
                float centerX = x + w * 0.5;
                float centerY = y + h * 0.5;// 计算相对中心点的归一化坐标
